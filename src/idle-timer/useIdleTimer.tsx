...
export function useIdleTimer({
	...
	useEffect(() => {
		...
		if (promptTimeout >= MAX_TIMEOUT) {
			throw new Error(
				`❌ The value for the promptTimeout property must fit in a 32 bit signed integer, ${MAX_TIMEOUT}.`
			);
		}

		if (promptBeforeIdle >= MAX_TIMEOUT) {
			throw new Error(
				`❌ The value for the promptBeforeIdle property must fit in a 32 bit signed integer, ${MAX_TIMEOUT}.`
			);
		}

		if (promptBeforeIdle >= timeout) {
			throw new Error(
				`❌ The value for the promptBeforeIdle property must be less than the timeout property, ${timeout}.`
			);
		}
	 ...
	 */
	const createTimeout = (time?: number, setLastActive = true): void => {
		destroyTimeout();
	...
	const eventHandler = (event: EventType): void => {
		...
		if (!idle.current && immediateEventsRef.current.includes(event.type as EventsType)) {
	 ...
	 * @private
	 */
	const unbindEvents = (force = false): void => {
	 ...
	 */
	const start = useCallback<(remote?: boolean) => void>(
		(remote?: boolean): void => {
			// Clear timeout
			destroyTimeout();
	 ...
	 */
	const reset = useCallback<(remote?: boolean) => void>(
		(remote?: boolean): void => {
			// Clear timeout
			destroyTimeout();
	 ...
	 */
	const activate = useCallback<(remote?: boolean) => void>(
		(remote?: boolean): void => {
			// Clear timeout
			destroyTimeout();
	 ...
	 *
	 */
	const pause = useCallback<(remote?: boolean) => boolean>(
	 ...
	 */
	const resume = useCallback<(remote?: boolean) => boolean>(
		(remote = false): boolean => {
			// Timer is not paused
			if (!paused.current) return false;
			paused.current = false;
	...
	useEffect(() => {
		if (!firstLoad.current) {
			const newEvents = [...new Set([...events, ...immediateEvents]).values()];
			unbindEvents();
