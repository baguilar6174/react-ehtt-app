...
export class Polyfill {
	...
	get onmessage() {
		const onmessage = this.mc.port1.onmessage;
		if (!onmessage) throw new Error('error onmessage');
		return onmessage;
	}
	...
	get onmessageerror() {
		const onmessageerror = this.mc.port1.onmessageerror;
		if (!onmessageerror) throw new Error('error onmessageerror');
...
/* export const BroadcastChannel =
	typeof window === 'undefined'
		...
		: Polyfill; */
export const BroadcastChannel =
	typeof typeof window.BroadcastChannel === 'function' ? window.BroadcastChannel : Polyfill;
